...
func (d *SQL) indexFromKeys(k []string, o interface{}) (interface{}, error) {
	if getObjectType(o) == ArrayObj {
		if strings.HasPrefix(k[0], "[") && strings.HasSuffix(k[0], "]") {
			intVar, err := strconv.Atoi(k[0][1 : len(k[0])-1])
			if err != nil {
				return nil, err
			}
			return d.getPath(k[1:], o.([]interface{})[intVar])
		}
	}
